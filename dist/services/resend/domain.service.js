...
class ResendDomainService {
     ...
     */
    static async createDomain(domainName, region = "us-east-1", enableReceiving = true) {
        try {
     ...
     */
    static async findOrCreateDomain(domainName, enableReceiving = true) {
        const clean = domainName.toLowerCase().trim().replace(/^https?:\/\//, "").replace(/\/.*$/, "");
     ...
     */
    static async getEmailMetrics(domainId, domainName = "example.com", startDate, endDate, organizationId) {
        ...
        const orgApiKey = env_js_1.env.RESEND_ORG_API || process.env.RESEND_ORG_API || env_js_1.env.RESEND_API_KEY || process.env.RESEND_API_KEY;
        if (orgApiKey) {
            try {
                const client = this.getClient();
                ...
                if (!effectiveDomainId && cleanDomain && cleanDomain !== "example.com") {
                    try {
                        ...
                        const matching = domainList.find((d) => d.name?.toLowerCase() === cleanDomain);
                        if (matching?.id) {
                ...
                const rawRes = await client.emails.metrics(metricsOptions);
                ...
                if (metricsObj && metricsObj.totals) {
                    ...
                    const filteredData = (metricsObj.data || [])
                        .filter((d) => !effectiveDomainId || !d.domain_id || d.domain_id === effectiveDomainId || d.domain_name === cleanDomain)
                        .map((d) => ({
                        ...d,
        ...
        try {
            ...
            const baseFilter = { createdAt: { $gte: start, $lte: end } };
            if (organizationId) {
