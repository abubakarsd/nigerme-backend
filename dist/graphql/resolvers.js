...
exports.resolvers = {
    ...
    Mutation: {
        ...
        rescheduleMail: async (_, { id, scheduledAt }, context) => {
            const authUser = (0, context_js_1.requireAuth)(context);
            const email = await index_js_7.EmailModel.findOne({
                _id: id,
                organizationId: authUser.organizationId,
            ...
            if (!email)
                throw new Error("Email not found");
            if (email.resendId) {
                const updateRes = await index_js_6.ResendEmailService.rescheduleEmail(email.resendId, scheduledAt);
        ...
        cancelScheduledMail: async (_, { id }, context) => {
            const authUser = (0, context_js_1.requireAuth)(context);
            const email = await index_js_7.EmailModel.findOne({
                _id: id,
                organizationId: authUser.organizationId,
            ...
            if (!email)
                throw new Error("Email not found");
            if (email.resendId) {
                const cancelRes = await index_js_6.ResendEmailService.cancelScheduledEmail(email.resendId);
