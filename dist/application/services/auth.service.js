...
class AuthService {
    ...
    static async login(dto) {
        ...
        if (user.twoFactorEnabled && user.phone) {
            await otp_service_js_1.OtpService.sendPhoneOtp(user.phone, "login_2fa");
            return {
     ...
     */
    static async verify2faAndLogin(phone, code) {
        await otp_service_js_1.OtpService.verifyPhoneOtp(phone, code, "login_2fa");
        const user = await user_model_js_1.UserModel.findOne({ phone });
        if (!user) {
