...
export const INITIAL_PACKAGES = [
  {
    ...
    description:
      "Enterprise-grade sovereign mailbox with custom domain SPF/DKIM/DMARC routing, task management, smart scheduling, video meetings, and encrypted cloud drive.",
  ...
  },
  {
    ...
    description:
      "Full Nigerian statutory compliance engine with direct NIBSS bank batch transfers, automated PAYE/Pension calculations, employee self-service payslip portal, and leave tracking.",
    category: "FINANCE_HR",
    ...
    pricingModel: "FLAT_MONTHLY" as const,
  ...
  },
  {
    ...
    description:
      "Modern retail and wholesale inventory control system coupled with an offline-capable Point of Sale register, barcode scanner integration, stock replenishment alerts, and profit margin analytics.",
  ...
  },
  {
    ...
    description:
      "Full-cycle dispatch management platform for 3PLs, e-commerce fleets, and courier companies. Includes digital waybill issuance, live GPS delivery tracking, automated customer SMS updates, and proof-of-delivery signatures.",
    category: "LOGISTICS",
    badge: "Supply Chain",
    badgeTone: "warning",
    isCore: false,
  ...
  },
  {
    ...
    description:
      "All-in-one hospitality property management system (PMS) for boutique hotels, guest houses, and resort chains. Features interactive room reservation grid, guest folio billing, online booking portal, and housekeeping sync.",
 ...
 */
export async function seedPackages(forceUpdate = false): Promise<void> {
  try {
    const existingCount = await PackageModel.countDocuments();
    if (existingCount === 0 || forceUpdate) {
      console.log(`🌱 [Database Seeder] Seeding ${INITIAL_PACKAGES.length} product packages into MongoDB...`);
