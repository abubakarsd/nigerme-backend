...
export const DEFAULT_ORG_ROLES = [
  {
    ...
    permissions: {
      ...
      canAccessHotel: true,
      canAccessCrm: true,
      canManageCrmCustomers: true,
      canManageCrmTickets: true,
      canManageCrmDeals: true,
      canManageCrmQuotes: true,
  ...
  },
  {
    ...
    permissions: {
      ...
      canAccessCrm: false,
      canManageCrmCustomers: false,
      canManageCrmTickets: false,
      canManageCrmDeals: false,
      canManageCrmQuotes: false,
  ...
  {
    name: "CRM Sales & Support Agent",
    slug: "crm_agent",
    description: "Access to support tickets, sales deals, customer profiles, and quote generation.",
    isSystem: false,
    memberCount: 0,
    permissions: {
      canAccessEmail: true,
...
export async function seedOrganizationDefaultDepartments(orgId: mongoose.Types.ObjectId | string) {
  try {
    const existingDepts = await DepartmentModel.find({ organizationId: orgId });
    if (existingDepts.length > 0) return existingDepts;

    const deptsToCreate = DEFAULT_ORG_DEPARTMENTS.map((d) => ({
      ...d,
      organizationId: orgId,
      memberCount: 0,
    }));

    const created = await DepartmentModel.insertMany(deptsToCreate);
